      ...
      >
        {
          (label !== false && type !== 'radio' && type !== 'checkbox')
            ? <label className="field__label" dangerouslySetInnerHTML={{ __html: (placeholder && variant === 'dynamic-placeholder') ? placeholder : label }} />
            : ''
        }
        <input className="field__native" type={type} name={name} required={required} pattern={pattern} placeholder={(variant !== 'dynamic-placeholder') ? placeholder : ''} {...attrs} />
        {
          (type === 'radio' || type === 'checkbox')
            ? <label className="field__label" dangerouslySetInnerHTML={{ __html: label }} />
            : ''
        }
      </Tag>
    );
